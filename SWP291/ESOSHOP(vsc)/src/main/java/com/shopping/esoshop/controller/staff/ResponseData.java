...
public class ResponseData {
    ...
    public ResponseEntity<String> createProduct(Model model, HttpSession session,
            ...
            @RequestParam(value = "supplier", defaultValue = "0") Integer supplier,
        ...
        if (name.isEmpty() || content.isEmpty() || category == 0 || brand == 0 || price == 0 || quantity == 0
                || colorId.isEmpty() || colorname.isEmpty()) {
    ...
    public ResponseEntity<String> createColor(HttpSession session,
        ...
        if (productId.isEmpty() || colorId.isEmpty() || colorname.isEmpty()) {
            return ResponseEntity.ok().body("Color add false");
        } else {
            ...
            for (Color cl : product.getColor()) {
                if (cl.getColorname().contains(colorname)) {
                    clere=true;
                }
            }
            if(!clere){
            ...
            return ResponseEntity.ok().body("Color have add ");
        }
    }
    ...
    @GetMapping("/staff/product/update/{productid}/{status}")
    ...
    @PostMapping("/staff/product/category/add")
    public ResponseEntity<Boolean> addcategoryes(@ModelAttribute Category category ){
        if(category.getName().trim().isEmpty())return  ResponseEntity.ok().body(false);
        else return ResponseEntity.ok().body(daoService.addNewCategory(category));  
    }

    @PostMapping("/staff/product/brand/add")
    public ResponseEntity<Boolean> addcategoryes(@ModelAttribute Brand brand ){
